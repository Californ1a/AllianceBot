...
const del = (chan, m) => {
	chan.bulkDelete(m).then(messages => {
		send(chan, `Purged ${messages.size} messages.`);
	}).catch(console.error);
};
...
exports.run = (bot, msg, args) => {
	...
	if (!args[0]) {
		...
		console.log("(args[0] && isNaN(args[0]))", (args[0] && isNaN(args[0])));
		console.log("(!isNaN(args[0]) && parseInt(args[0]) <= 1)", (!isNaN(args[0]) && parseInt(args[0]) <= 1));
		console.log("(!msg.mentions.users.first() && !args[1])", (!msg.mentions.users.first() && !args[1]));
